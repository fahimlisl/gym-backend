...
import { isSuperAdmin } from "../middlewares/isSuperAdmin.middlewares.js";
import {
  destroyTrainer,
  editTrainer,
  fetchAllTrainer,
...
import {
  ...
  fetchSubPlans,
  removeBenefits,
} from "../controllers/plans.controllers.js";
...
import {
  ...
  updateWorkoutStatus,
} from "../controllers/assignedWorkout.controllers.js";
...
router.route("/renewalSubscription/:id/:planId").patch(verifyJWT, isAdmin, renewalSubscription);
...
router.route("/fetchAllCafeOrders").get(verifyJWT, isAdmin, fetchEveryCafeOderOfAllStuff);
...
router.route("/destroyCafeAdmin/:id").delete(verifyJWT, isAdmin, destroyCafeAdmin);

// cafe items
router.route("/add-item").post(upload.single("image"), verifyJWT, isAdmin, addCafeItem);
...
router.route("/fetchParticularCafeItem/:id").get(verifyJWT, isAdmin, fetchParticularCafeItem);
router.route("/toggleAvailability/:id").patch(verifyJWT, isAdmin, toggleAvailabilty);
...
router.patch("/diet/approve/:dietId", verifyJWT, isAdmin, approveDiet);
