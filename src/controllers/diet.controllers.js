...
const editCalories = asyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, calories, "Calories updated successfully"));
});
...
const setDietMacros = asyncHandler(async (req, res) => {
  ...
  if (Math.abs(macroCalories - diet.calories) > 50) {
    throw new ApiError(400, "Macro calories do not match total calories");
  }
  ...
  res.json(new ApiResponse(200, diet, "Macros set successfully"));
});
...
const createMeal = asyncHandler(async (req, res) => {
  ...
  if (!meal) throw new ApiError(400, "Meal name is required");

  const diet = await Diet.findById(dietId);
  if (!diet) throw new ApiError(404, "Diet not found");

  const exists = diet.meals.find((m) => m.meal === meal);
  if (exists) throw new ApiError(400, "Meal type already exists");
  ...
  return res
    .status(200)
    .json(new ApiResponse(200, diet, "Meal added successfully"));
});

const removeMeal = asyncHandler(async (req, res) => {
  const { mealId, dietId } = req.params;

  if (!mealId) throw new ApiError(400, "Meal ID is required");
  ...
  if (!updatedDiet) throw new ApiError(400, "Failed to remove meal");

  return res
    .status(200)
    .json(new ApiResponse(200, updatedDiet, "Meal removed successfully"));
});
...
const foodItemInserction = asyncHandler(async (req, res) => {
  ...
  if (!foodDocs.length) throw new ApiError(400, "Foods not found");
  ...
  const updatedDiet = await Diet.findOneAndUpdate(
    { user: userId, "meals._id": mealId },
    { $push: { "meals.$.foods": { $each: preparedFoods } } },
    { new: true }
  );

  if (!updatedDiet) throw new ApiError(500, "Failed to update diet");

  return res
    .status(200)
    .json(new ApiResponse(200, updatedDiet, "Foods added successfully"));
});
...
const removeItemFromDiet = asyncHandler(async (req, res) => {
  ...
  const diet = await Diet.findOneAndUpdate(
    { user: userId, "meals._id": mealId },
    { $pull: { "meals.$.foods": { foodId: new mongoose.Types.ObjectId(foodId) } } },
    { new: true }
  );

  if (!diet) throw new ApiError(400, "Failed to update diet");

  return res
    .status(200)
    .json(new ApiResponse(200, diet, "Food removed successfully"));
});
...
const approveDiet = asyncHandler(async (req, res) => {
  ...
  const message =
    status === "draft" ? "Diet unlocked for editing" : "Diet approved successfully";
...
const checkIfDietExists = asyncHandler(async (req, res) => {
  ...
  if (!diet) {
    return res
      .status(200)
      .json(new ApiResponse(200, { exists: false }, "No diet found for this user"));
  }

  return res.status(200).json(
    new ApiResponse(200, { exists: true, dietId: diet._id, status: diet.status }, "Diet exists")
  );
});
